...
def _normalize_modulo_return(raw, num_exps: int) -> np.ndarray:
    ...
    if arr.size != num_exps:
        raise RuntimeError(
            f"Expected {num_exps} counts, got size={arr.size}, shape={arr.shape}"
        )
...
# -------------------------
def _run_seq_blocking_swab(
    pulsegen, seq_file: str, seq_args_string: str, num_reps: int
) -> int:
    """
                ...
                for step_ind in step_ind_list:
                    ...
                    try:
                        ...
                        if stream_load_in_run_fn:
                            ...
                            period_ns = int(
                                pulsegen.stream_load(seq_file, seq_args_string)[0]
                            )
                            time.sleep(
                                min((period_ns * 1e-9) * int(num_reps) + 0.05, 60.0)
                            )
                        else:
                            _run_seq_blocking_swab(
                                pulsegen, seq_file, seq_args_string, int(num_reps)
                            )
                        ...
                        crash_counter[run_ind] += 1
                        crash_log[run_ind].append(
                            (int(step_ind), traceback.format_exc())
                        )
