...
class Dmd6500:
    ...
    def send_image_mono(self, start_plane: int, image_u8: np.ndarray) -> None:
        ...
        self._check(
            self.lib.SendImageMono(self._require_handle(), start_plane, ptr),
            "SendImageMono",
        )
    ...
    def send_binary_plane(
        self, plane_nr: int, binary_image: np.ndarray, bitlevel: int = 0
    ) -> None:
        plane = self.calc_binary_plane(binary_image, bitlevel=bitlevel)
        ptr = plane.ctypes.data_as(ctypes.POINTER(ctypes.c_ubyte))
        self._check(
            self.lib.SendPlane(self._require_handle(), int(plane_nr), ptr), "SendPlane"
        )
    ...
    def upload_sequence(
        self, commands: Iterable[int], startpos: int = 1000
    ) -> np.ndarray:
    ...
    def run_sequence(self, startpos: int) -> None:
        self._check(
            self.lib.RunSequence(self._require_handle(), int(startpos)), "RunSequence"
        )
...
def find_dmd_xy_for_camera_spots_blocking(
    ...
    if plot:
        plot_response_curves(
            x_positions,
            x_drop,
            spot_indices=range(min(5, len(cam_pts))),
            title="DMD x drop scan",
        )
...
def check_zero_order_block(
    ...
    residual_fraction = (
        zero_after_val / zero_before_val if zero_before_val > 0 else np.nan
    )
...
def radius_scan_for_spot_blocking(
    ...
    if plot:
        ...
        plt.axvline(
            best_radius, color="r", linestyle="--", label=f"best r={best_radius}"
        )
...
def refine_triangle_centers_and_radii_blocking(
    ...
    print(
        f"Approx mirrors flipped per spot ≈ {np.pi * recommended_movie_radius**2:.0f}"
    )
    print("=" * 80)
