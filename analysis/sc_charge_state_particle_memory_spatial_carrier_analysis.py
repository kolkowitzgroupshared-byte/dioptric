# -*- coding: utf-8 -*-
"""
Spatial carrier-event analysis v5 with multi-file-per-wait appending.
...
def _build_pair_geometry(coords_um):
    ...
    tri_i, tri_j = np.triu_indices(n_nv, k=1)
    delta = coords_um[tri_i] - coords_um[tri_j]
    dist = np.sqrt(np.sum(delta * delta, axis=1))

    edges = np.arange(
        0.0,
        PAIR_MAX_DISTANCE_UM + PAIR_BIN_WIDTH_UM + 1e-12,
        PAIR_BIN_WIDTH_UM,
    )

    # Keep only pairs inside the configured range.
    ...
    return {
        "edges_um": edges,
        "centers_um": centers,
        "pair_counts_geometry": np.asarray(pair_counts, dtype=int),
        "adjacency": adjacency,
    }
...
def _plot_pair_correlation(ds, cut, result, pair_geometry):
    ...
    x = pair_geometry["centers_um"]
    obs = result["observed"]["C"]
    ...
    ax.set_title(
        f"{ds['wait_s']:g} s | Lambda >= {cut:.3f} | "
        f"{len(result['event_payloads'])} events | global p={result['global_max_p']:.3g}"
    )
    ax.legend(fontsize=8)
    ...
    if SAVE_OUTPUTS:
        fig.savefig(
            OUTPUT_DIR
            / (
                f"pair_correlation_{ds['wait_s']:g}s_"
                f"{_cut_token(cut)}.png"
            ),
            dpi=190,
            bbox_inches="tight",
...
def _summary_text(
    ...
    lines = []
    lines.append("SPATIAL CARRIER-EVENT ANALYSIS V5")
    lines.append("=" * 92)
    lines.append("")
    lines.append(
    ...
    for cut in LAMBDA_H_CUTS:
        ...
        for ds in datasets:
            ...
            best = max(sub, key=lambda r: r["C_z_vs_null"])

            lines.append(
                f"  {ds['wait_s']:g} s: max positive uniform-residual bin "
                f"{best['distance_low_um']:.1f}-{best['distance_high_um']:.1f} um; "
...
def main():
    ...
    print("\n" + "#" * 118)
    print("SPATIAL CARRIER-EVENT ANALYSIS V5")
    print("#" * 118)
    print(
