...
if __name__ == "__main__":
    ...
    # Compact cross-wait diagnostic figures: one row/panel per physical wait.
    compact_figures = {}
    if (
        (MAKE_COMPACT_DARK_WAIT_TRACE_GRID or MAKE_COMPACT_DARK_WAIT_DISTRIBUTION_GRID)
        and len(individual_analyses) >= 1
    ):
        compact_figures = _make_compact_dark_wait_figures(individual_analyses)

    # Charge-state-threshold stability across all source measurements.
    threshold_consistency_rows = []
    threshold_consistency_figures = {}
    if MAKE_CHARGE_THRESHOLD_CONSISTENCY_PLOTS and len(individual_analyses) >= 1:
        (
            threshold_consistency_rows,
            threshold_consistency_figures,
    ...
    # Keep compact figures first; retain standard comparison figures as additional
    # summary products.  Per-wait figure dictionaries are included only when the
    # user explicitly enables MAKE_INDIVIDUAL_WAIT_FIGURES.
    figures = {}
    ...
    # Convenient interactive handle for the numerical consistency table.
    threshold_consistency = threshold_consistency_rows
